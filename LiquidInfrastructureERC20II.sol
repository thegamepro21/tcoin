    ...
    ReentrancyGuard
{
    ...
    //@audit anyoone can call this function
    function distribute(uint256 numDistributions) public nonReentrant {
        ...
        for (i = nextDistributionRecipient; i < limit; i++) {
            ...
            if (isApprovedHolder(recipient)) {
                ...
                for (uint j = 0; j < distributableERC20s.length; j++) {
                    ...
                    //@audit users wont get anything due to multiplication by 0
        ...
        nextDistributionRecipient = limit;//+++++++++++++
        //@audit if endDistribution is not triggered, LockedForDistribution remains true
        //@audit this wont be triggered even if nextDistributionRecipent == holders.length
    ...
    function _isPastMinDistributionPeriod() internal view returns (bool) {
        ...
        //@audit if last distribution is not set, this will always pass
    ...
    function _beginDistribution() internal {
        ...
        if (erc20EntitlementPerUnit.length > 0) {
            delete erc20EntitlementPerUnit;//@audit performed twice remove one to save gas
        }
        ...
        for (uint i = 0; i < distributableERC20s.length; i++) {
            ...
            uint256 entitlement = balance / supply;//@audit rounding to zero, when supply > balance
            erc20EntitlementPerUnit.push(entitlement);
        }
        //@audit nextDistributionRecipent does not get any token from new distribution
        //if he diidnt get token from previous distrubution
     ...
     */
    function burnFromAndDistribute(address account, uint256 amount) public { //@audit access control
        if (_isPastMinDistributionPeriod()) {
            distributeToAllHolders();
        }
    ...
    function withdrawFromManagedNFTs(uint256 numWithdrawals) public {
        //@audit if lockedforDistribution stays true, withdrawal is impossible
        ...
        nextWithdrawal = i;//-----
        nextWithdrawal = i;//+++++++
        //@audit under no condition would this be triggered
